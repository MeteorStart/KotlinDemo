...
object TabLayoutHelper {
     ...
     * @email: lx802315@163.com
     */
    fun setUpIndicatorWidth(tabLayout: TabLayout, padding: Float) {
        val tabLayoutClass = tabLayout.javaClass
        var tabStrip: Field? = null
        try {
            tabStrip = tabLayoutClass.getDeclaredField("mTabStrip")
        ...
        try {
            ...
            for (i in 0 until layout!!.childCount) {
                ...
                val params = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.MATCH_PARENT, 1f)
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    params.marginStart = DisplayManager.dip2px(padding)!!
                    params.marginEnd = DisplayManager.dip2px(padding)!!
                }
                child.layoutParams = params
                child.invalidate()
            }
